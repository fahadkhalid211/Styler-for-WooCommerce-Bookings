...
(function($) {
	...
	window.WCBSWizard = {
		...
		init: function() {
			...
			$wizard.find('.wcbs-wizard-next-btn').on('click', function(e) {
				...
				if (self.validateStep(self.currentStep, $wizard)) {
					self.goToStep(self.currentStep + 1, $wizard);
				}
			});
		...
		validateStep: function(step, $wizard) {
			if (step === 2) {
				// Check if date and slot are chosen
		...
		updateStepVisibility: function($wizard) {
			...
			var $calSec = $wizard.find('.wc-bookings-date-picker, .wcbs-calendar-mockup, .wcbs-slot-tabs-nav, .block-picker, .wcbs-slots-container');
			// Step 3: Persons / Quantities
			var $personsSec = $wizard.find('.wc-bookings-booking-form fieldset:not(.wc-bookings-date-picker)');
